...
class ImportDialog(QDialog):
    ...
    Позволяет настраивать импорт DXF-объектов в PostGIS.
    """
    ...
    def setup_ui(self):
        ...
        Logger.log_message("Настройка интерфейса диалога импорта")
        ...
        self.export_settings_group = QGroupBox(self.lm.get_string("EXPORT_DIALOG", "import_settings_group"))
        ...
        self.enable_column_mapping_checkbox = QCheckBox(self.lm.get_string("EXPORT_DIALOG", "enable_column_mapping"))
        self.enable_column_mapping_checkbox.setToolTip(self.lm.get_string("EXPORT_DIALOG", "column_mapping_tooltip"))
        ...
        self.configure_mapping_button = QPushButton(self.lm.get_string("EXPORT_DIALOG", "configure_mapping_button"))
        ...
        # Чекбокс "Импортировать только слои"
        self.export_layers_only_checkbox = QCheckBox(self.lm.get_string("EXPORT_DIALOG", "import_layers_only"))
        ...
        self.export_button = QPushButton(self.lm.get_string("EXPORT_DIALOG", "import_button"))
    ...
    def _connect_signals(self):
        ...
        self.export_button.clicked.connect(self.on_import_clicked)
    ...
    def refresh_data_dialog(self):
        ...
        Logger.log_message("Обновление данных диалога импорта")
    ...
    def show_window(self):
        ...
        Logger.log_message("Отображение диалогового окна импорта")
    ...
    def check_export_enabled(self):
        """Проверяет, можно ли включить кнопку импорта"""
        # Основные требования
        basic_requirements = (
        ...
        # Если импортируем только слои, схема файлов не нужна
        if self.export_layers_only:
            schema_requirements = layer_schema_selected
        else:
            # Если импортируем файлы тоже, нужны обе схемы
        ...
        if not can_export:
            if not basic_requirements:
                Logger.log_message("Импорт недоступен: не настроено подключение к БД")
            elif not schema_requirements:
                Logger.log_message("Импорт недоступен: не выбраны необходимые схемы")

    def on_cancel_clicked(self):
        """Обработка нажатия кнопки Отмена"""
        Logger.log_message("Импорт отменен пользователем")
    ...
    def on_import_clicked(self):
        ...
        # Сохраняем текущие настройки подключения и схем перед импортом
        self.save_current_connection()

        # Создаем и настраиваем диалог прогресса
        self.progress = QProgressDialog(self.lm.get_string("EXPORT_DIALOG", "import_progress"), None, 0, 0, self)
        self.progress.setWindowModality(Qt.WindowModal)
        self.progress.setWindowTitle(self.lm.get_string("EXPORT_DIALOG", "import_title"))
        ...
        Logger.log_message(f"Выбран режим маппирования для импорта: {mapping_mode}")
        ...
        Logger.log_message(f"Название файла для импорта: '{final_filename}' (оригинальное: '{selected_file_name}')")
        ...
        if not export_success:
            Logger.log_error("Не удалось создать временный DXF файл для импорта")
        ...
        Logger.log_message(f"Создан временный файл для импорта: {temp_filename}")
        ...
        Logger.log_message(f"Импорт только слоев: {self.export_layers_only}")
        # Создаем импортер и получаем поток импорта
        ...
        # Сохраняем имя временного файла для удаления после завершения импорта
    ...
    def on_import_finished(self, success, message):
        ...
        # Удаляем временный файл после завершения импорта
        ...
        if success:
            ...
            Logger.log_error(f"Ошибка при импорте: {message}")
            QtWidgets.QMessageBox.critical(
                self,
                self.lm.get_string("EXPORT_DIALOG", "error_title"),
                self.lm.get_string("EXPORT_DIALOG", "import_error", message)
            )
    ...
    def load_schemas(self):
        ...
        # Проверяем возможность импорта после загрузки схем
        self.check_export_enabled()
    ...
    def on_export_layers_only_toggled(self, checked):
        """Обработка изменения чекбокса 'Импортировать только слои'"""
        ...
        Logger.log_message(f"Импорт только слоев: {checked}")
        self.check_export_enabled()
