# -*- coding: utf-8 -*-
"""
Main Dialog (Refactored) - облегчённый главный диалог.
...
from .import_dialog_refactored import ImportDialogRefactored
...
class ConverterDialogRefactored(QtWidgets.QDialog, FORM_CLASS):
    """
    Главный диалог плагина DXF-PostGIS Converter.
    
    Рефакторированная версия — тонкий координатор, использующий DI.
    """
    ...
    def __init__(
        ...
        Logger.log_message("MainDialog (refactored) initialized")
    ...
    def _on_import_to_db(self):
        ...
        # Открываем рефакторированный диалог импорта
        dialog = ImportDialogRefactored(
            self.dxf_handler,
            self.dxf_tree_widget_handler,
            self._container,
            self
