...
class DXFToPostGISConverter:
    def convert_entity_to_postgis(self, entity: DXFEntity) -> Optional[Dict]:
        ...
        conversion_functions = {
            'POINT': self._convert_point_to_postgis,
            'LINE': self._convert_line_to_postgis,
            'POLYLINE': self._convert_polyline_to_postgis,
            'LWPOLYLINE': self._convert_lwpolyline_to_postgis,
            'TEXT': self._convert_text_to_postgis,
            'CIRCLE': self._convert_circle_to_postgis,
            'ARC': self._convert_arc_to_postgis,
            'MULTILEADER': self._convert_multileader_to_postgis,
            'INSERT': self._convert_insert_to_postgis,
            #'3DSOLID': self._convert_3dsolid_to_postgis,
            'SPLINE': self._convert_spline_to_postgis,
            'ELLIPSE': self._convert_ellipse_to_postgis,
            'MTEXT': self._convert_mtext_to_postgis,
            'SOLID': self._convert_solid_to_postgis,
            'TRACE': self._convert_trace_to_postgis,
            '3DFACE': self._convert_3dface_to_postgis,
            'REGION': self._convert_region_to_postgis,
            'BODY': self._convert_body_to_postgis,
            'MESH': self._convert_mesh_to_postgis,
            'HATCH': self._convert_hatch_to_postgis,
            'LEADER': self._convert_leader_to_postgis,
            'SHAPE': self._convert_shape_to_postgis,
            'VIEWPORT': self._convert_viewport_to_postgis,
            'IMAGE': self._convert_image_to_postgis,
            'IMAGEDEF': self._convert_imagedef_to_postgis,
            'DIMENSION': self._convert_dimension_to_postgis,
            'RAY': self._convert_ray_to_postgis,
            'XLINE': self._convert_xline_to_postgis,
            'ATTRIB': self._convert_attrib_to_postgis,
            'SEQEND': self._convert_seqend_to_postgis,
            'HELIX': self._convert_helix_to_postgis,
            #'VERTEX': self._convert_vertex_to_postgis,
        }
        ...
        return {
            'geometry': geometry, 
            'geom_type': geom_type, 
            'extra_data': self._verify_extra_data(extra_data),
            'notes': extra_data.get('text', None)  # Add notes field from text if available
        }

    def _replace_vec3_to_list(self, data):
        if isinstance(data, dict):
            return {key: self._replace_vec3_to_list(value) for key, value in data.items()}
        elif isinstance(data, list):
            return [self._replace_vec3_to_list(item) for item in data]
        elif isinstance(data, Vec3):
            return [data.x, data.y, data.z]
        else:
            return data

    def _list_to_vec3(self, data):
        """
        ...
        """
        if isinstance(data, dict):
            return {key: self._list_to_vec3(value) for key, value in data.items()}
        elif isinstance(data, list):
            ...
            return [self._list_to_vec3(item) for item in data]
        else:
            return data

    def _verify_extra_data(self, data):
        data = self._replace_vec3_to_list(data)
        return data

    def _attributes_to_dict(self, entity: DXFEntity, data: dict = None) -> dict:
        if data is None:
            data = {}
