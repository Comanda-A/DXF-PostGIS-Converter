...
def ShapeCodepage2Utf8(OrgShpDat, TargetShpDat, OrgCodePage):
    ...
    if myQGIS_VERSION_INT() < 31003:
        zLayer = QgsVectorFileWriter.writeAsVectorFormat(oLayer, TargetShpDat, TargetCodePage, oLayer.crs(),
                                                         "ESRI Shapefile")
    else:
        options = QgsVectorFileWriter.SaveVectorOptions()
        options.driverName = "ESRI Shapefile"
        options.fileEncoding = TargetCodePage
        zLayer = QgsVectorFileWriter.writeAsVectorFormatV2(oLayer, TargetShpDat, QgsCoordinateTransformContext(),
                                                           options)


def attTableEdit(sOutForm, inpDat, bFormat, sCharSet, gpkgTable=None, txtErsatz4Tab=' '):
    if sCharSet == "System":
        sCharSet = locale.getlocale()[1]
                ...
                for param in params:
                    ...
                    if sArt == "LABEL":
                        ...
                        if AktText is None:
                            ...
                            if bDecodeError:
                                if aktHandle == None:
                                    addFehler(tr("Wrong char in  'ogr_style' at EntityHandle: ") + tr(
                                        " (Check your choose charset)") + tryDecode(dummy, sCharSet))
                                else:
                                    addFehler(tr("Wrong char in  'ogr_style' at EntityHandle: ") + aktHandle + tr(
                                        " (Check your choose charset)") + tryDecode(dummy, sCharSet))
