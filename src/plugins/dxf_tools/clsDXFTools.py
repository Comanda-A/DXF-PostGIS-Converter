...
def labelingDXF(qLayer, bFormatText, bUseColor4Point, dblFaktor):
    ...
    sf = "%.1f" % dblFaktor
    sf = sf + " * \"size\""
    qLayer.setCustomProperty("labeling/dataDefined/OffsetXY",
        ...
        if antw != QMessageBox.Yes:
            ...
            for dat in delDatArr:
                try:
                    ...
                    QMessageBox.critical(None, tr("DZD:file remove error"),
...
def DXFImporter(uiParent, sOutForm, listDXFDatNam, zielPfadOrDatei, bZielSave, sCharSet, bCol, bLayer, bFormatText,
                bUseColor4Point, bUseColor4Line, bUseColor4Poly, dblFaktor, chkTransform, DreiPassPunkte, bGen3D,
                txtErsatz4Tab):
    import processing
    ...
    try:
        ...
        for i in range(listDXFDatNam.count()):
            ...
            Antw = EineDXF(uiParent, mLay.crs(), bZielSave, sOutForm, grpProjekt, AktList, Kern, AktOpt, AktDXFDatNam,
                           zielPfadOrDatei, qPrjDatName, sCharSet, bLayer, bFormatText, bUseColor4Point, bUseColor4Line,
                           bUseColor4Poly, dblFaktor, okTransform, DreiPassPunkte, bGen3D, txtErsatz4Tab)
    except:
        subLZF()
...
def EineDXF(uiParent, mLay_crs, bZielSave, sOutForm, grpProjekt, AktList, Kern, AktOpt, DXFDatNam, zielPfadOrDatei,
            qPrjDatName, sOrgCharSet, bLayer, bFormatText, bUseColor4Point, bUseColor4Line, bUseColor4Poly, dblFaktor,
            chkTransform, DreiPassPunkte, bGen3D, txtErsatz4Tab):
    import processing
    from processing.core.Processing import Processing
        ...
        if pAntw is None:
            ...
            if bKonvOK:
                ...
                if sOutForm == "SHP":
                    ...
                    addFehler(
                        tr("Creation '%s' failed. Please look to the QGIS log message panel (OGR)") % korrGPKGDatNam)
